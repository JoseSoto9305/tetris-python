...
class  Spinner(Button):
    ...
    def __init__(self, key, function, draw_on):
        ...
        surface, rect = config.text_objects(self.text, left=self.x, top=self.y, font_size=config.medium_text)
        ...
        if self.button_on:
            ...
            while running:
                ...
                for i in range(len(self.vals)):
                    surface, rect = config.text_objects(f'{self.vals[i]}', x_center=cx, y_center=cy + self.h/2 + i * self.h, 
                                                        color=color_text, font_size=self.vals_font_size)
                    ...
                    if rect_button.collidepoint(pos):
                        if click[0] == 1:
                            ...
                            running = False                            
                        surface, rect = config.text_objects(f'{self.vals[i]}', x_center=cx, y_center=cy + self.h/2 + i * self.h, 
                                                        color=color_text_act, font_size=self.vals_font_size*2)
                        self.screen.blit(surface, rect)
